package eu.monnetproject.clesa;
...
import eu.monnetproject.clesa.commons.lucene.utils.AnalyzerFactory;
import eu.monnetproject.clesa.commons.lucene.utils.Searcher;
import eu.monnetproject.clesa.commons.utils.Pair;
import eu.monnetproject.clesa.commons.utils.TextNormalizer;
import eu.monnetproject.clesa.commons.utils.Vector;
import eu.monnetproject.clesa.commons.utils.VectorUtils;
...
public class CLESA implements CLSim {
	...
	public double score(String text1, Language lang1, String text2, Language lang2) {
		if(text1!=null && lang1!=null && text2!=null && lang2!=null) {
			Pair<String, Language> pair1 = new Pair<String, Language>(text1.trim(), lang1);
			Pair<String, Language> pair2 = new Pair<String, Language>(text2.trim(), lang2);		
			return score(pair1, pair2);
		} 
		return -0.0;	
	}
	...
	public double score(Pair<String, Language> pair1, Pair<String, Language> pair2) {
		...
		new TopicVectorThread("vector1", null, vectorMap, pair1, searcher).run();
		new TopicVectorThread("vector2", null, vectorMap, pair2, searcher).run();
		//barrier.barrierWait();		
	...
	public Vector<String> getVector(Pair<String, Language> pair) {
		...
		new TopicVectorThread("vector", null, vectorMap, pair, searcher).run();
		//barrier.barrierWait();		
		Vector<String> vector = vectorMap.get("vector");
		return vector;
	}

        private final Map<Integer,String> uriCache = Collections.synchronizedMap(new HashMap<Integer,String>());
		...
		public void getVector() {
			...
			for(int i=0;i<scoreDocs.length;++i) {
				...
				score = scoreDocs[i].score;
                                ...
                                final boolean containsDocID;
                                synchronized(uriCache) {
                                    containsDocID = uriCache.containsKey(docID);
                                }
                                if(containsDocID) {
                                    synchronized(uriCache) {
                                        uri = uriCache.get(docID);
                                    }
                                } else {
                                    org.apache.lucene.document.Document document = searcher.getDocumentWithDocID(docID);
                                    uri = document.get("URI_EN");
                                    synchronized(uriCache) {
                                        uriCache.put(docID, uri);
                                    }
                                }
